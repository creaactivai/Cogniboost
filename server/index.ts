...
async function runStartupMigrations() {
  try {
    ...
    // Work Plans — Phase 1.3 ESL Roadmap. One curated 21-day plan per
    // student per cycle, generated by Claude based on real submission data.
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_work_plans_user_status ON work_plans (user_id, status)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_work_plans_created ON work_plans (created_at DESC)`);
    console.log('Startup migrations: work_plans table verified');
  } catch (err) {
